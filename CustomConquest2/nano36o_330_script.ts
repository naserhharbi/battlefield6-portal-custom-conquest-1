...
let MinimumDistanceToDetectEnemies = 25; //meters.
...
class PlayerClass
{
  ...
  // Flag to indicate an AI Bot should be redeployed after spawning. Used when an AI Bot leaves the game.
...
function CheckForEnemies(player:PlayerClass):boolean
{
  let myPosition = mod.GetObjectPosition(player.player);
  let enemies = Players.getAliveEnemyTeamMates(player.team);
  return CheckForEnemiesUpdate(myPosition, enemies);
}
function CheckForEnemiesUpdate(myPosition:mod.Vector, enemies:PlayerClass[]):boolean
{
